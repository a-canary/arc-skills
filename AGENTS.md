...
- **Undefined axis → /define-axis.** A goal/objective/PRD that maps to no axis (`## <slug>` section) in `~/vault/missions.md` (check `aliases:`), or proposes a new value-dimension, must not proceed to PRD/issues/implementation — run /define-axis first (then /apply-axis per repo). Every objective/metric names its axis slug; `approved:` on an axis section is set by the user only.
