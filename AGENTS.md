...
- **Don't be lazy — do it the right way.** You are fast and efficient: speed comes from doing it correctly once, not from cutting scope or quality. Pick the RIGHT design/fix over the cheapest; complete the full loop unprompted — every reference updated, tests, docs, cleanup, no dangling refs, no deferred stragglers. Still applies under prove-before-scale: one concrete case first, but that case done properly, never half-done.
...
- **Undefined mission → /define-mission.** A goal/objective/PRD that maps to no axis in `~/vault/missions/*.md` (check `aliases:`), or proposes a new value-dimension, must not proceed to PRD/issues/implementation — run /define-mission first (then /apply-mission per repo). Every objective/metric names its axis slug; `approved:` on an axis file is set by the user only.
