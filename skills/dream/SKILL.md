...
The Task tool's subagent loader only honors `model: opus|sonnet|haiku|inherit`. A provider alias here (e.g. `minimax`) silently falls back to the parent model — so the collector MUST be `haiku` (cheap tier) and the adapter SHOULD be `opus` (capable tier). See SETUP.md for the full wiring note.
