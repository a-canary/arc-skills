...
If you skip the picker, the shipped defaults are `model: haiku` for collector
and `model: opus` for adapter. **These names must resolve in the Task-tool
subagent loader**, which honors `opus | sonnet | haiku | inherit` only. A
provider alias here (e.g. `minimax`) silently falls back to `inherit` and
burns the parent's model tokens on bulk journal paging — see commit 7588f08
for the fix history.
