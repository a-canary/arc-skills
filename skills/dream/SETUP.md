...
environment. The Task-tool subagent loader honors `opus | sonnet | haiku |
inherit`; a provider alias like `minimax` here silently falls back to
`inherit` and burns Opus-class tokens on bulk paging.
