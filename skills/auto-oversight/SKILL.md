...
- **onenation** (repos renamed to `ndivisible` 2026-09-03; the rotation key stays `onenation` — that's what `state.json`, the ledger and `/m/onenation` use): TWO checkouts — deploy/prod `/home/aaron/projects/ndivisible` AND director-dev `/home/aaron/repos/ndivisible` (the 2h director cron plans from repos/; verify BOTH synced to origin/main or the director plans against stale code). Docker stack, QA journey cron log `.claude/qa-journey-cron.log`, deploy-drift cron `~/vault/ndivisible/deploy-drift.log`, open PRs, `/m/onenation`
