...
Run the [[grill-protocol]] — then add domain-model challenges as you go.

## Domain awareness

When the user uses a term, challenge it against `CONTEXT.md`. When a term is
resolved, update `CONTEXT.md` right there. Offer ADRs only when the decision is
...
up — capture them as they happen. Use the format in [CONTEXT-FORMAT.md](./CONTEXT-FORMAT.md).

`CONTEXT.md` should be totally devoid of implementation details. It is a
glossary and nothing else. Do not treat it as a spec, a scratch pad, or a
repository for implementation decisions.
