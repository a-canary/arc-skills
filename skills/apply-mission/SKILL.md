---
name: apply-mission
description: Propose, for user approval, how a global mission applies to ONE repo — a CHOICES.md "## Mission:" interpretation, phase-ordered objectives, and one hillclimb(scope, metric, gate) eval per phase — landed via the apply-on-approve CHOICES.md PR lane. Use when a repo lacks an interpretation for a relevant mission, after /define-mission's repo walk, or when the user says "apply the mission to this repo", "apply the mission", "propose objectives", or "add metric gates".
---
...
Read the mission file + repo state (README, CHOICES.md, ADRs, recent work).
Relevance check first: if the mission genuinely doesn't apply, record an `n/a`
...
A `## Mission: <slug>` section for the repo's CHOICES.md:

- **Interpretation** — 2-4 sentences: how this repo serves the mission, which
  mission values bind here and how (vetoes made concrete, skews named).
...
Every objective/metric row NAMES the UM-ID — dashboards aggregate by
mission; an untagged objective is a lint gap.
...
approved, the mission shows "uninterpreted" for this repo and only global
vetoes bind.
